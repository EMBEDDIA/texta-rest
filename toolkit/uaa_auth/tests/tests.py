...
class UAATests(APILiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.only_admin_user = "only_admin"
    ...
    def test(self):
        ...
        self.run_login_with_refresh_and_access_token_success(TEST_UAA_USERNAME, TEST_UAA_PASSWORD)
        # Username and password is the same, password isn't returned however so it can't be referenced properly.
        self.run_login_with_refresh_and_access_token_success(username=self.only_admin_user["username"], password=self.only_admin_user["username"])
    ...
    def __parse_csrf_token(self, uaa_login_url):
    ...
    def create_users(self):
        ...
        csrf_token = self.__parse_csrf_token(uaa_login_url=uaa_login_url)
        ...
        try:
            ...
            self.fail("Request should nicely turn into an ConnectionError, it does not.")
            ...
            bodies = [
                ...
                # Create user for testing login with only texta_admin scope.
                {
                    "userName": self.only_admin_user,
                    "name": {
                        "formatted": "given name family name",
                        "familyName": "Admin",
                        "givenName": "User"
                    ...
                    "emails": [{
                        "value": "admin.user@test.org",
                        "primary": True
                    }],
                    "active": True,
                    "verified": True,
                    "origin": "",
                    "password": self.only_admin_user
                }
            ]

            for body in bodies:
                self.__create_uaa_user(headers, body)


    def __create_uaa_user(self, headers, body):
        username = body['userName']
        json_data = json.dumps(body)
        create_resp = requests.post(f'{UAA_URL}/Users', headers=headers, data=json_data)
        # Replace the username inside the class object with the response payload for easier access of additional info like id etc.
        setattr(self, f"{username}_user", json.loads(create_resp.content))
        print_output(f"run_create_{username}_user:resp", create_resp)
        self.assertEqual(201, create_resp.status_code)


    def __create_group(self, headers, body):
        json_data = json.dumps(body)
    ...
    def create_groups(self):
        ...
        csrf_token = self.__parse_csrf_token(uaa_login_url)
        ...
        try:
            ...
            self.fail("Not supposed to reach this part.")
    ...
    def run_login_with_refresh_and_access_token_success(self, username: str, password: str):
        """
    ...
    def run_user_is_not_superuser_without_admin_group(self):
        ...
        try:
            ...
            print_output(
                "run_user_is_not_superuser_without_admin_group:user",
                user_resp.data
            )
