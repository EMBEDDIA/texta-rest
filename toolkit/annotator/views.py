...
from toolkit.annotator.serializers import AnnotatorProjectSerializer, AnnotatorGroupSerializer, AnnotatorSerializer, BinaryAnnotationSerializer, CommentSerializer, \
    DocumentEditSerializer, DocumentIDSerializer, EntityAnnotationSerializer, LabelsetSerializer, MultilabelAnnotationSerializer, RecordSerializer, PullDocumentSerializer
...
class AnnotatorViewset(mixins.CreateModelMixin,
    ...
    def _enrich_document_with_meta(self, document: dict, user: User, annotator: Annotator):
        ...
        document["_source"][TEXTA_ANNOTATOR_KEY]["document_counter"] = meta_data.get("document_counter", 0)
        return document
    ...
    def pull_document(self, request, pk=None, project_pk=None):
        ...
        document = annotator.pull_document(document_counter)

        if document:
            document = self._process_document_output(document, request.user, annotator)
            return Response(document)
        else:
            detail_message = "No such document!" if document_counter else "No more documents left!"
    ...
    def pull_skipped(self, request, pk=None, project_pk=None):
        ...
        document_counter = serializer.validated_data["document_counter"]
        document = annotator.pull_skipped_document(document_counter)
        if document:
            document = self._process_document_output(document, request.user, annotator)
            return Response(document)
        else:
    ...
    def skip_document(self, request, pk=None, project_pk=None):
        ...
        processed_timestamp = document["_source"].get("processed_timestamp_utc", None)
